...
Based on what we see here the data is saying that the more children that are on the Farms program the higher the math scores are. But, this is only slightly and there is not much consistency to support making a definitive statement that the Farms program raises test scores. 
...
Because the residuals are so high, we cannot be confident that this model is telling the whole story. The data does not line up well with the model in this circumstance. Additionally, the p value is extremely low. The model is not worth using. It tells us that there is a positive relationship between FARMS percentage and math scores, but we cannot be certain that is true based on the issues above. 
...
There is a large difference in the predicted result vs. actual math scores for Whitemarsh Elementary School, with the predicted being 71 and the actual being 26.
...
From this models results, I would not say that elementary schools with fewer students on free and reduced price meals do better on standardized math tests. You have to look at other factors influencing the math scores. The more affluent areas, who do not need the FARMS program, also have access to better education which increases there math scores. Only considering the two factors creates a positive realtionship on a graph that is not true to the situation. Context is very important. 